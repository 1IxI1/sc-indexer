...
from contracts_db.database import (
    Account,
    LPool,
    LPoolAsset,
    LPoolWithAssets,
...
#     res = [
#         BookingMinimalModel(utime=i[0], booking_type=i[1], debit=i[2], credit=i[3])
...
async def get_lpool(session: AsyncSession, lpool_address: str) -> Optional[LPoolModel]:
    """Returns liquidity pool data."""
    query = select(LPoolWithAssets).filter(LPoolWithAssets.account == lpool_address)
    res = await session.execute(query)
    ...
    assets = [
        LPoolAssetModel(
            ...
            reserve=pool.asset1_reserve,
        ),
        LPoolAssetModel(
            address=pool.asset2_address,
            symbol=pool.asset2_symbol,
            reserve=pool.asset2_reserve,
        ),
    ]
    return LPoolModel(type=pool.lpool_type, assets=assets)


async def get_pools_by_assets(
    session: AsyncSession, asset1: str, asset2: Optional[str] = None
) -> Optional[List[LPoolModel]]:
    """Returns all liquidity pools for given asset or pair."""

    query = select(LPoolWithAssets)
    if asset2:
        query = query.filter(
            or_(
                and_(
                    LPoolWithAssets.asset1_address == asset1,
                    LPoolWithAssets.asset2_address == asset2,
                ),
                # and reverse:
                and_(
                    LPoolWithAssets.asset2_address == asset1,
                    LPoolWithAssets.asset1_address == asset2,
                ),
            )
        )
    else:
    ...
    res = await session.execute(query)
    pools_got = res.scalars().all()

    if not pools_got:
        return None

    pools_res = []
    for pool in pools_got:
        assets = [
            LPoolAssetModel(
                address=pool.asset1_address,
                symbol=pool.asset1_symbol,
        ...
        pools_res.append(LPoolModel(type=pool.lpool_type, assets=assets))
    return pools_res
