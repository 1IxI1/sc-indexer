"""
Handler for stonfi liquidity pool smart contract.
...
from pytoniq_core.boc import Address, Cell, ExternalAddress, Slice
...
async def handle_stonfi_pool(
    ...
    try:
        res = await lite_client.run_get_method(lpool_addr_str, "get_pool_data", [])
    except Exception as e:
        logger.error(f"Failed to run get_assets on {lpool_addr_str}: {e}")
        return

    async def parse_asset_addr(addr: Slice):
        addr_class = addr.load_address()
        if addr_class is None or isinstance(addr_class, ExternalAddress):
            raise TypeError(f"Invalid address: {addr}")
        addr_raw = addr_class.to_str(False)
        symbol = ""
        try:
            ...
            logger.error(f"Failed to get symbol for {addr_raw}: {e}")
        return {
            "type": "jetton",
            "addr": addr_raw,
            "symbol": symbol,
    ...
    asset0 = await parse_asset_addr(res[2])
    asset1 = await parse_asset_addr(res[3])
    ...
    await result_conn.commit()
    logger.info(f"Lpool (stonfi) {lpool_addr_str} was added")
    return 
...
stonfi_pool_handler = (
    "glZq1ytlaP5ydkN9OwyRGqtl7XAcE2AZQbKRcwXoHBE=",
    handler,
)
